...
import logicbox.proof.OptionalProof.Line
import logicbox.proof.OptionalProof.Box
import scala.annotation.meta.getter

object OptionalProof {
  ...
  def empty[F, R, B, Id]: OptionalProof[F, R, B, Id] = 
    OptionalProof(steps = Map.empty, rootSteps = Nil)
}

case class OptionalProof[F, R, B, Id](
  steps: Map[Id, OptionalProof.Step[F, R, B, Id]],
  rootSteps: List[Id],
) extends ModifiableProof[Option[F], Option[R], Option[B], Id]
{
  ...
  private def getStepImpl(id: Id): Option[OptionalProof.Step[F, R, B, Id]] = steps.get(id)
  ...
  private case class InsertResult(
    newStepSeq: List[Id], 
    modifiedSteps: List[(Id, OptionalProof.Step[F, R, B, Id])] = Nil
  )
  ...
  override def addLine(id: Id, where: Pos[Id]): Either[Error[Id], Pf] = for {
    ...
    pf <- insertStep(id, OptionalProof.Line(None, None, Nil), where)
  } yield pf

  override def addBox(id: Id, where: Pos[Id]): Either[Error[Id], Pf] = for {
    _ <- assertStepNotExists(id)
    pf <- insertStep(id, OptionalProof.Box(None, Nil), where)
  } yield pf

  private def getCurrentLine(lineId: Id): Either[Error[Id], OptionalProof.Line[F, R, Id]] = for {
  ...
  private def updateStep(stepId: Id, newStep: OptionalProof.Step[F, R, B, Id]): Pf =
    OptionalProof(rootSteps = rootSteps, steps = steps + (stepId -> newStep))
  ...
  private case class RemoveResult(
    newStepSeq: List[Id], 
    modifiedSteps: List[(Id, OptionalProof.Step[F, R, B, Id])]
  )
  ...
  private def computeReachableNodes(steps: List[Id], newMap: Map[Id, OptionalProof.Step[F, R, B, Id]]): Set[Id] = 
    val ls = for {
      id <- steps
      step <- newMap.get(id)
  ...
  } yield OptionalProof(rootSteps = newStepSeq, steps = newMap -- toRemove)
}
